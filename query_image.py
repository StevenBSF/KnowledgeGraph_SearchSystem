...
# 使用本地图片文件
query_img_path = 'query.jpg' # 假设图片位于当前工作目录下
query_raw_image = Image.open(query_img_path).convert('RGB')
query_image_inputs = processor(images=query_raw_image, return_tensors="pt", padding=True)
query_image_features = model.get_image_features(**query_image_inputs)
query_vector = query_image_features.detach().numpy()
